<?php
	...
	*/
	$aColumns = array( 'unit_id', 'unit_no', 'unit_name'); //Kolom Pada Tabel

	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'unit_id';

	// DB table to use
	$sTable = 'unit'; // Nama Tabel
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		$row = array();
		$btn = '<a href="#" onClick="showModals(\''.$aRow['unit_id'].'\')" class="btn btn-info btn-sm">Edit</a> | <a href="#" onClick="deleteUser(\''.$aRow['unit_id'].'\')" class="btn btn-danger btn-sm">delete</a>';
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			$row[] = $aRow[ $aColumns[$i] ];
		}
		$row = array( $btn, $no++, $aRow['unit_no'], $aRow['unit_name']);
		$output['aaData'][] = $row;
	}
