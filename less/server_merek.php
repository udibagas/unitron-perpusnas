<?php
	...
	*/
	$aColumns = array( 'brand_id', 'brand_name'); //Kolom Pada Tabel

	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'brand_id';

	// DB table to use
	$sTable = 'brands'; // Nama Tabel
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		$row = array();
		$btn = '<a href="#" onClick="showModals(\''.$aRow['brand_id'].'\')" class="btn btn-info btn-sm">Edit</a> | <a href="#" onClick="deleteUser(\''.$aRow['brand_id'].'\')" class="btn btn-danger btn-sm">delete</a>';
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			$row[] = $aRow[ $aColumns[$i] ];
		}
		$row = array( $btn, $no++, $aRow['brand_name']);
		$output['aaData'][] = $row;
	}
