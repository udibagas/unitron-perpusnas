<?php
	...
	*/
	$aColumns = array( 'country_code', 'country_name'); //Kolom Pada Tabel

	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'country_code';

	// DB table to use
	$sTable = 'countries'; // Nama Tabel
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		$row = array();
		$btn = '<a href="#" onClick="showModals(\''.$aRow['country_code'].'\')" class="btn btn-info btn-sm">Edit</a> | <a href="#" onClick="deleteUser(\''.$aRow['country_code'].'\')" class="btn btn-danger btn-sm">delete</a>';
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			$row[] = $aRow[ $aColumns[$i] ];
		}
		$row = array( $btn, $no++, $aRow['country_code'], $aRow['country_name']);
		$output['aaData'][] = $row;
	}
