<?php
	...
	*/
	$aColumns = array( 'lokasi_id', 'lokasi_no', 'lokasi_name'); //Kolom Pada Tabel

	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'lokasi_id';

	// DB table to use
	$sTable = 'lokasi'; // Nama Tabel
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		$row = array();
		$btn = '<a href="#" onClick="showModals(\''.$aRow['lokasi_id'].'\')" class="btn btn-info btn-sm">Edit</a> | <a href="#" onClick="deleteUser(\''.$aRow['lokasi_id'].'\')" class="btn btn-danger btn-sm">delete</a>';
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			$row[] = $aRow[ $aColumns[$i] ];
		}
		$row = array( $btn, $no++, $aRow['lokasi_no'], $aRow['lokasi_name']);
		$output['aaData'][] = $row;
	}
