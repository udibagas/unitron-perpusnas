...
(SELECT SUM(trans5.nilai)*220 as jumlah FROM trans5 INNER JOIN sensor ON trans5.ID_SENSOR = sensor.ID_SENSOR WHERE sensor.PARAMETER like 'Current%' AND sensor.POSISIDETAIL like '%MSB%'  AND trans5.id IN (SELECT MAX(id) AS id FROM trans5  GROUP BY ID_SENSOR )) as k_daya
