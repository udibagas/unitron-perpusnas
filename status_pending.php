                    ...
                    <div class="clearfix"></div>
                        ...
                        <div class="abcdefg">
                            <div class="panel panel-success">
                                ...
                                <h2>Rekap Permohonan Izin Yang Masih Pending Sebagai SKPD</h2>
 ...
 <input type="submit" class="btn btn-info" name="t1" value="Filter"/>
                            ...
                            if($_SESSION['level']=='user' ){
                                if(isset($_POST['tgl_awal'])==false){
                                    $idd = $_SESSION['id_req'];
                                $sql=$konek->query("SELECT * FROM request WHERE id_regis='$idd' AND status<>'S'  ORDER BY tanggal_masuk DESC");
                                }else{
                                ...
                                if($tgl1 != null && $tgl2!= null){
                                  ...
                                  $sql=$konek->query("SELECT * FROM request WHERE id_regis='$idd' AND tanggal_masuk BETWEEN '$tgl1' AND '$tgl2' AND (status<>'S') ORDER BY tanggal_masuk DESC");
                              }else{
                                $sql=$konek->query("SELECT * FROM request WHERE id_regis='$idd' AND (status<>'S')  ORDER BY tanggal_masuk DESC");
                              }
                            ...
                            } elseif($_SESSION['level']=='admin' || $_SESSION['level']=='pj') {


                                 if(isset($_POST['tgl_awal'])==false){
                                    $idd = $_SESSION['id_req'];
                                $sql=$konek->query("SELECT * FROM request WHERE (status<>'S')  ORDER BY tanggal_masuk DESC");
                                }else{
                                    $tgl1 = mysqli_real_escape_string($konek, $_POST['tgl_awal']);
                                    $tgl2 = mysqli_real_escape_string($konek, $_POST['tgl_akhir']);
                                if($tgl1 != null && $tgl2!= null){
                                  ...
                                  $sql=$konek->query("SELECT * FROM request WHERE tanggal_masuk BETWEEN '$tgl1' AND '$tgl2' AND (status<>'S') ORDER BY tanggal_masuk DESC");
                              }else{
                                $sql=$konek->query("SELECT * FROM request WHERE (status<>'S')  ORDER BY tanggal_masuk DESC");
                              }
                                 ...
                                 if(isset($_POST['tgl_awal'])==false){
                                    $idd = $_SESSION['id_req'];
                                $sql=$konek->query("SELECT * FROM request WHERE atasan='Y'  ORDER BY tanggal_masuk DESC");
                                }else{
                                    $tgl1 = mysqli_real_escape_string($konek, $_POST['tgl_awal']);
                                    $tgl2 = mysqli_real_escape_string($konek, $_POST['tgl_akhir']);
                                if($tgl1 != null && $tgl2!= null){
                                  ...
                                  $sql=$konek->query("SELECT * FROM request WHERE tanggal_masuk BETWEEN '$tgl1' AND '$tgl2' AND atasan='Y' ORDER BY tanggal_masuk DESC");
                              }else{
                                $sql=$konek->query("SELECT * FROM request WHERE atasan='Y'  ORDER BY tanggal_masuk DESC");
                              }
                                              ...
                                              <td class=" last"><h2><span class="fa fa-check"> Menunggu Konfirmasi</span></h2></td>
                                                ...
                                                <h2><span class="fa fa-times"> Batal</span></h2></td>
                                                    ...
                                                    <?php if($_SESSION['level']=='pj'){?>
                                                      <td><a class="btn btn-info" href="?page=<?php echo $data['step']; ?>&tipe=detail&id=<?php echo $data['id_req']; ?>&cekcek=on">Detail</a></td>
                                                      <?php }else{ ?>
                                                        <td><a class="btn btn-info" href="?page=<?php echo $data['step']; ?>&id=<?php echo $data['id_req']; ?>&cekcek=on">Detail</a></td>
                            ...
                            <?php } ?>
                                 ...
                                 <?php
                                    $qcek_pemandu = $_SESSION['pemandu'];

                                    if($qcek_pemandu=='Y'){
                                        include "status_pending_pemandu.php";
                                    }
                                    ?>
