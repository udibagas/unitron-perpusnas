    ...
    <script src="js/datetimepicker/bootstrap-datetimepicker.js"></script>
		...
		<script type="text/javascript" language="javascript" >
			...
			$(document).ready(function() {
				dTable = $('#example').DataTable( {
					...
					"bJQueryUI": false,
					"responsive": true,
					"sAjaxSource": "less/server_perangkat.php", // Load Data
					"sServerMethod": "POST",
					"columnDefs": [
			...
			<div class="col-md-1 col-sm-12 col-xs-12"></div>
				<div class="col-md-10 col-sm-12 col-xs-12">
					<button onClick="showModals()" class="btn btn-primary">Tambah Kode Perangkat</button>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="myModalLabel">Add Kode Perangkat</h4>
					</div>
					<div class="modal-body">
						...
						<form class="form-horizontal" id="formtim">
							...
							<div class="form-group">
								<label for="real_name" class="col-sm-2 control-label">No Perangkat</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="no_perangkat" name="no_perangkat" onkeypress="return isNumberKey(event)">
								</div>
							</div>
							<div class="form-group">
								<label for="npm" class="col-sm-2 control-label">Golongan Perangkat</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="gol_perangkat" name="gol_perangkat" >
								</div>
							</div>
		...
		<script>
			...
			function setModalData( data )
			{
				...
				$("#id").val(data.golongan_perangkat_id);
				$("#type").val("edit");
				$("#userid").val(data.golongan_perangkat_id);

				$("#no_perangkat").val(data.golongan_perangkat_no);
				$("#gol_perangkat").val(data.golongan_perangkat_name);
				$("#myModals").modal("show");
			}
			...
			function deleteUser( id )
			{
				...
				$.ajax({
					...
					success: function(data) {
						...
						$("#no_perangkat").val(data.golongan_perangkat_no).attr("disabled","true");
						$("#gol_perangkat").val(data.golongan_perangkat_name).attr("disabled","true");
			...
			function clearModals()
			{
				...
				$("#no_perangkat").val("").removeAttr( "disabled" );
				$("#gol_perangkat").val("").removeAttr( "disabled" );
