    ...
    <script src="js/datetimepicker/bootstrap-datetimepicker.js"></script>
		...
		<script type="text/javascript" language="javascript" >
			...
			$(document).ready(function() {
				dTable = $('#example').DataTable( {
					...
					"bJQueryUI": false,
					"responsive": true,
					"sAjaxSource": "less/server_sub_ruang.php", // Load Data
					"sServerMethod": "POST",
					"columnDefs": [
			...
			<div class="col-md-1 col-sm-12 col-xs-12"></div>
				<div class="col-md-10 col-sm-12 col-xs-12">
					<button onClick="showModals()" class="btn btn-primary">Tambah Sub Ruang</button>
					...
					<div class="modal-body">
						...
						<form class="form-horizontal" id="formtim">
							...
							<div class="form-group">
								<label for="real_name" class="col-sm-2 control-label">Ruang</label>
								<div class="col-sm-10">
									<select class="form-control" id="no_ruang" name="no_ruang">
										
									<option>Pilih Lokasi</option>

									<?php
										$lokasi = $konek->query('SELECT * FROM ruang');
										while($data=$lokasi->fetch_assoc()){
											echo "<option value='$data[ruang_id]'>$data[ruang_name] </option>";
										}
							...
							<div class="form-group">
								<label for="real_name" class="col-sm-2 control-label">No sub Ruang</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="no_sub_ruang" name="no_sub_ruang" >
								</div>
							</div>
							<div class="form-group">
								<label for="npm" class="col-sm-2 control-label">Nama Sub Ruang</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="nama_sub_ruang" name="nama_sub_ruang" >
								</div>
							</div>
		...
		<script>
			...
			function setModalData( data )
			{
				...
				$("#id").val(data.sub_ruang_id);
				$("#type").val("edit");
				$("#userid").val(data.sub_ruang_id);

				$("#no_ruang").val(data.ruang_id);
				$("#no_sub_ruang").val(data.sub_ruang_no);
				$("#nama_sub_ruang").val(data.sub_ruang_name);
				$("#myModals").modal("show");
			}
			...
			function deleteUser( id )
			{
				...
				$.ajax({
					...
					success: function(data) {
						...
						$("#no_ruang").val(data.ruang_id).attr("disabled","true");
						$("#no_sub_ruang").val(data.sub_ruang_no).attr("disabled","true");
						$("#nama_sub_ruang").val(data.sub_ruang_name).attr("disabled","true");
			...
			function clearModals()
			{
				...
				$("#no_sub_ruang").val("").removeAttr( "disabled" );
				$("#nama_sub_ruang").val("").removeAttr( "disabled" );
