     <?php
    $sql = $konek->query("SELECT sensor.ID_SENSOR,sensor.POSISIDETAIL,AVG(trans5.nilai) as rata,sensor.NILAIMIN,sensor.NILAIMAX FROM sensor INNER JOIN trans5 ON trans5.ID_SENSOR = sensor.ID_SENSOR WHERE sensor.PARAMETER='Suhu' AND LEFT(sensor.POSISIDETAIL,3)='RAK' AND sensor.NAMALOKASI='$id' GROUP BY sensor.ID_SENSOR");
...
timeTicket<?php echo $b; ?> = setIntervalAndExecute(function (){
            $.getJSON("less/jsensor_suhu_rak_sensor.php?id=<?php echo $data['ID_SENSOR']; ?>", function(json) {
                 if(json===null){
                      var nilai = 0;
                  ...
                  if(nilai>max || nilai<min){
                        document.getElementById("suhu_sensor_rak<?php echo $b; ?>").className="btn btn-danger btn-xs blink_me";
                  }else{
                        document.getElementById("suhu_sensor_rak<?php echo $b; ?>").className="";
                  }
                }
