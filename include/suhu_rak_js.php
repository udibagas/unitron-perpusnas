    ...
    FROM
    sensor
    WHERE sensor.PARAMETER='Suhu' AND LEFT(sensor.POSISIDETAIL,3)='RAK'
            ...
            $.getJSON("less/jsensor_suhu_rak.php?id=<?php echo $data['POSISIDETAIL']; ?>", function(json) {
                if(json===null){
                  ...
                  if(nilai>max || nilai<min){
                        document.getElementById("suhu_rak<?php echo $b; ?>").className="btn btn-danger btn-xs blink_me";
                  }else{
                        document.getElementById("suhu_rak<?php echo $b; ?>").className="";
                  }
                }
