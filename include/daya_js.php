...
FROM
sensor
WHERE left(sensor.PARAMETER,6)='Active' 
GROUP BY sensor.ID_SENSOR,sensor.PARAMETER ORDER BY sensor.STATUSALAT,sensor.ID_SENSOR ASC ");
              ...
              $.getJSON("less/jsensor_daya.php?id=<?php echo $data['ID_SENSOR']; ?>", function(json) {
                 if(json===null){
                  ...
                  if(nilai>max || nilai<min){
                        document.getElementById("daya_lok<?php echo $b; ?>").className="btn btn-danger btn-xs blink_me";
                  }else{
                        document.getElementById("daya_lok<?php echo $b; ?>").className="";
                  }
                }
               ...
               $.getJSON("less/jsensor_daya_total.php?id=<?php echo $data['POSISIDETAIL']; ?>", function(json) {
                 if(json===null){
                      var sum = 0;
                    ...
                    var sum = json['sum'];
                    var load = json['MAX_LOAD'];
                    var persen = sum/load*100;

                    if(load==null){
                      document.getElementById("value<?php echo $data['POSISIDETAIL']; ?>").innerHTML = sum +' KW';
                    }else{
                      document.getElementById("value<?php echo $data['POSISIDETAIL']; ?>").innerHTML = sum + ' / '+ load +' KW';
                    }

                 
                  document.getElementById("progress_bar<?php echo $data['POSISIDETAIL']; ?>").className="ui-progress-bar";
                  
                  document.getElementById("bar<?php echo $data['POSISIDETAIL']; ?>").style.width=persen+'%';
                }
              });
