 ...
 <input type="submit" class="btn btn-info" name="t1" value="Filter"/>
                            ...
                            <?php
                                ...
                                if(isset($_POST['tgl_awal'])==false){
                                    $idd = $_SESSION['id_req'];
                                $sql=$konek->query("SELECT request.id_req,request.tanggal_masuk,request.tanggal_keluar,request.tujuan,request.`status`,request.pendamping,request.akses_ruang,request.file,pemandu.waktu,pemandu.keterangan,pemandu.`status` FROM request INNER JOIN pemandu ON request.id_req = pemandu.id_req  WHERE pemandu.id_regis='$idd'  ORDER BY tanggal_masuk DESC");
                                }else{
                                ...
                                if($tgl1 != null && $tgl2!= null){
                                  ...
                                  $sql=$konek->query("SELECT request.id_req,request.tanggal_masuk,request.tanggal_keluar,request.tujuan,request.`status`,request.pendamping,request.akses_ruang,request.file,pemandu.waktu,pemandu.keterangan,pemandu.`status` FROM request INNER JOIN pemandu ON request.id_req = pemandu.id_req  WHERE pemandu.id_regis='$idd' AND tanggal_masuk BETWEEN '$tgl1' AND '$tgl2'  ORDER BY tanggal_masuk DESC");
                              }else{
                                $sql=$konek->query("SELECT request.id_req,request.tanggal_masuk,request.tanggal_keluar,request.tujuan,request.`status`,request.pendamping,request.akses_ruang,request.file,pemandu.waktu,pemandu.keterangan,pemandu.`status` FROM request INNER JOIN pemandu ON request.id_req = pemandu.id_req  WHERE pemandu.id_regis='$idd'  ORDER BY tanggal_masuk DESC");
                              }
                                 ...
                                 $nr = $sql->num_rows;
                                 if($nr<1){
                                 $sql=$konek->query("SELECT request.id_req,request.tanggal_masuk,request.tanggal_keluar,request.tujuan,request.`status`,request.pendamping,request.akses_ruang,request.file,register.nama,register.alamat,register.email,register.instansi FROM request INNER JOIN register ON request.id_regis = register.id_register WHERE request.id_regis='$idd'  ORDER BY tanggal_masuk DESC");
                              }
                            ...
                            ?>
                                           <tr class="odd pointer">
                                                ...
                                                <td><?php echo $data['tanggal_keluar']; ?></td>
                                                      ...
                                                      <td><a class="btn btn-default" href="?page=p5pp&tipe=detail&id=<?php echo $data['id_req']; ?>">Detail</a>
                                                    <?php if($kode=="buku"){ ?>
                                                      <a class="btn btn-info" href="?page=itinerary&id=<?php echo $data['id_req']; ?>">Update Itinerary</a><?php } ?></td>
